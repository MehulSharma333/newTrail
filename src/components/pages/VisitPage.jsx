...
import { getUserSoundsById } from '../../database/getUserSoundsById';
import { getFavouriteSoundsByUserId } from '../../database/getFavouriteSoundsByUserId';
...
const VisitPage = (props) => {
    ...
    const getSounds = useCallback(async (id) => {
        if (!sharedData) {
            const user = await getUserById(id);
            setUserData(user);
        }
        const allSounds = await getUserSoundsById(id);
        setCreatedSoundsOfUser(allSounds);
        const favSounds = await getFavouriteSoundsByUserId(id);
        setFavouriteSounds(favSounds.sounds);
    }, [sharedData]);

    useEffect(() => {
        getSounds(props.userId);
    }, []);
